...
- MongoDB DB_CHANNEL cache — Fast DB support
- Premium / Free plan system with download limits
- Per-user 1-hour video auto-deletion
- Progress bars for download & upload
- Channel membership gate
- Admin panel: addpremium, removepremium, checkuser, broadcast, stats
- Flask health-check server
- Webhook + Polling support
- Appicrypt signed API requests to DiskWala
"""
...
active_tasks         = {}   # user_id -> deque of task_ids
task_state           = {}   # task_id -> {"cancelled": bool}
active_uploads       = {}   # task_id -> True

uploader_clients     = {}   # name -> pyrogram Client
uploader_state       = {}   # name -> busy bool
uploader_lock        = asyncio.Lock()

user_active_messages = {}   # user_id -> [msg_id, ...]
...
# ─────────────────────────────────────────
# Stats (JSON fallback when no MongoDB)
# ─────────────────────────────────────────
class StatsStore:
...
def format_eta_speed(start_time: float, done: int, total: int):
    ...
    eta = int((total - done) / speed_bps) if (total and speed_bps > 0) else 0
...
# ─────────────────────────────────────────
# Uploader bot management (Pyrogram)
# ─────────────────────────────────────────
pyro_app = None  # main pyrogram client, initialized in startup
...
async def init_uploader_bots():
    ...
    try:
        ...
        uploader_clients["main"] = pyro_app
        uploader_state["main"]   = False
        logger.info("Main pyrogram client started.")
    except Exception as e:
        logger.error("Main pyrogram client failed: %s", e)
    ...
    for idx, token in enumerate(SUPPORT_BOT_TOKENS, start=1):
        ...
        try:
            ...
            logger.error("Support bot %s failed: %s", name, e)
...
# ─────────────────────────────────────────
# Progress (ptb-style, for telegram-bot)
# ─────────────────────────────────────────
...
class Progress:
    """Progress tracker for ptb (python-telegram-bot) messages."""
...
# ─────────────────────────────────────────
# Core: DiskWala download
# ─────────────────────────────────────────
async def resolve_diskwala_file(file_id: str) -> tuple[str, dict]:
    """
    Returns (download_url, info_dict).
    Tries video stream → regular download → sign endpoint.
    """
    info = {}
    try:
...
async def download_diskwala_file(
    ...
    """Download a DiskWala file to temp_dir. Returns (file_path, info)."""
...
async def upload_to_db_and_send(
    ...
    """
    1. Upload file to DB_CHANNEL.
    2. Save to MongoDB.
    3. Copy to user.
    4. Schedule 1-hour deletion.
    """
    start_time = time.time()
    last_edit  = [0]
    EDIT_INTERVAL = 4
    ...
    # Copy to user
    try:
        user_copy = await uploader_clients.get("main", uploader_client).copy_message(
            chat_id=user_id,
    ...
    # Track for deletion
    async with user_active_lock:
        lst = user_active_messages.setdefault(user_id, [])
        lst.append(user_copy.id)

    main_client = uploader_clients.get("main", uploader_client)
    asyncio.create_task(
        schedule_user_video_deletion(main_client, user_id, user_copy.id, delay=3600)
    )
    return user_copy


# ─────────────────────────────────────────
# Send file without DB_CHANNEL (fallback)
# ─────────────────────────────────────────
async def send_file_direct_ptb(message, progress: Progress, file_path: Path, info: dict):
    """Send downloaded file directly to Telegram (PTB, no DB_CHANNEL)."""
...
# ─────────────────────────────────────────
# Main task processor (Pyrogram path)
# ─────────────────────────────────────────
async def process_task_pyro(
    ...
    """Full pipeline for Pyrogram client: resolve → DB check → download → upload → send."""
    try:
        ...
        if cached and DB_CHANNEL:
            try:
                await status_msg.edit_text(f"⚡ **Fast DB** — cache se bhej raha hoon!\n📁 `{file_name}`")
        ...
        # 4. Download
        dest_path = os.path.join(TMP_DIR, f"{task_id}_{file_name}")
        try:
            async with global_active_sem:
                await download_diskwala_file(
                    file_id=file_id,
                    temp_dir=Path(TMP_DIR),
                    task_id=task_id,
                    progress_msg=status_msg,
                )
                # rename to correct dest_path
                downloaded_file = list(Path(TMP_DIR).glob(f"*{file_id}*"))
                if downloaded_file:
                    dest_path = str(downloaded_file[0])
        ...
        if not uploader_name:
            for _ in range(12):
                ...
                if uploader_name:
                    break
        if not uploader_name:
            await status_msg.edit_text("❌ Sabhi uploaders busy hain. Thodi der baad try karein.")
            _cleanup_path(dest_path)
            return
...
if API_ID and API_HASH and BOT_TOKEN:
    ...
    @pyro_bot.on_message(filters.command("start") & filters.private)
    ...
    @pyro_bot.on_message(filters.command("help") & filters.private)
    ...
    @pyro_bot.on_message(filters.command("status") & filters.private)
    ...
    @pyro_bot.on_message(filters.command("plans") & filters.private)
    ...
    @pyro_bot.on_message(filters.command("cancel") & filters.private)
    ...
    @pyro_bot.on_message(filters.command("addpremium") & filters.private)
    async def pyro_cmd_addpremium(client: Client, msg: Message):
        ...
        try:
            await client.send_message(
                target_id, f"🎉 Aapka premium plan activate ho gaya! ({label})\nFast downloads enjoy karein!"
            )
        except Exception:
            pass

    @pyro_bot.on_message(filters.command("removepremium") & filters.private)
    ...
    @pyro_bot.on_message(filters.command("checkuser") & filters.private)
    ...
    @pyro_bot.on_message(filters.command("broadcast") & filters.private)
    ...
    @pyro_bot.on_message(filters.command("stats") & filters.private)
    ...
    # Callback handlers
    @pyro_bot.on_callback_query(filters.regex(r"^cancel:(.+)$"))
    ...
    @pyro_bot.on_callback_query(filters.regex(r"^buyplan:(\d+):(-?\d+)$"))
    ...
    @pyro_bot.on_callback_query(filters.regex(r"^show_plans$"))
    ...
    @pyro_bot.on_callback_query(filters.regex(r"^check_join$"))
    ...
    @pyro_bot.on_message(filters.private & filters.text)
    async def pyro_on_text(client: Client, msg: Message):
        ...
        # Keyboard shortcuts
        if text == "💎 Plans":
            await pyro_cmd_plans(client, msg)
            return
        if text == "📊 My Status":
            await pyro_cmd_status(client, msg)
            return
        if text == "❓ Help":
            await pyro_cmd_help(client, msg)
            return
        if text == "📞 Support":
            await msg.reply_text(
        ...
        if REQUIRED_CHANNEL_USERNAME:
            ...
            if not is_member:
                await msg.reply_text(
                    f"❌ **Join required!**\n\n"
                    f"Pehle channel join karein:\n{REQUIRED_CHANNEL_URL or f'https://t.me/{REQUIRED_CHANNEL_USERNAME}'}",
                    reply_markup=join_kb_pyrogram(),
                )
                return

        # Queue check
        q = active_tasks.setdefault(user_id, deque())
    ...
    pyro_bot = None  # Pyrogram disabled


# ─────────────────────────────────────────
# PTB (python-telegram-bot) handlers
# — used when Pyrogram is NOT configured
# ─────────────────────────────────────────
...
async def ptb_handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if REQUIRED_CHANNEL_USERNAME and not await is_joined_ptb(context, user.id):
        ...
        txt = f"⚠️ Pehle channel join karein:\n{REQUIRED_CHANNEL_URL}"
        await msg.reply_text(
            txt,
            reply_markup=TgMarkup([
...
async def ptb_handle_check_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_joined_ptb(context, user.id):
        ...
        if pending:
            ...
            status   = await query.message.reply_text("🔍 Processing...", parse_mode="HTML")
            progress = Progress(status)
            temp_dir = DOWNLOAD_DIR / f"dw_{uuid.uuid4().hex}"
            temp_dir.mkdir(parents=True, exist_ok=True)
            async with dl_semaphore:
                try:
                    ...
                    await progress.done_dl()
                    await send_file_direct_ptb(query.message, progress, file_path, info)
                    await safe_edit(status, "✅ <b>Done!</b> Enjoy 🎉")
                except Exception as e:
                    await safe_edit(status, f"❌ <b>Failed!</b>\n\n<code>{e}</code>")
                finally:
...
async def main():
    ...
    if pyro_bot:
        # ── Pyrogram mode (full featured) ──
        await init_uploader_bots()

        # Replace default pyro_bot with the one started in init_uploader_bots
        # (they share the same bot token so we just use pyro_bot directly)
        await pyro_bot.start()
        me = await pyro_bot.get_me()
        logger.info("Pyrogram bot started: @%s", me.username)
        ...
        try:
            ...
            for name, cl in uploader_clients.items():
                if name != "main":
                    try:
                        await cl.stop()
                    except Exception:
