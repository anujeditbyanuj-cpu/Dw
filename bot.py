...
Fixes applied:
  ...
  #11 send_file_direct_ptb — reply_* replaced with bot.send_* + chat_id param
"""
...
MONGO_DB_URI = os.environ.get("MONGO_DB_URI", "mongodb+srv://Anujedit:Anujedit@cluster0.7cs2nhd.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
async def upload_to_db_and_send(
    ...
    try:
        ...
        # FIX #5 — thumb delete karo (finally mein safe hai dono cases ke liye)
...
# ─────────────────────────────────────────
# FIX #11 — send_file_direct_ptb
# reply_* hata ke bot.send_* use kiya
# chat_id explicitly parameter mein liya
# ─────────────────────────────────────────
async def send_file_direct_ptb(bot, chat_id: int, progress: Progress, file_path: Path, info: dict):
            ...
            with open(file_path, "rb") as f:
                if ext in VIDEO_EXTS:
                    if file_size > TG_STANDARD_LIMIT:
                        ...
                        await bot.send_video(chat_id=chat_id, video=f, **kw)
                    ...
                    if file_size > TG_STANDARD_LIMIT:
                        ...
                        if duration:
                            kw["duration"] = duration
                        await bot.send_audio(chat_id=chat_id, audio=f, **kw)
...
async def ptb_handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with dl_semaphore:
        try:
            ...
            # FIX #11 — bot aur chat_id explicitly pass karo
            await send_file_direct_ptb(context.bot, msg.chat_id, progress, file_path, info)
            await stats.increment()
...
# FIX #2 — deleted message pe reply_text fix
# FIX #10 — double query.answer() fix
# FIX #11 — send_file_direct_ptb ke liye bot + chat_id pass
    ...
    if await is_joined_ptb(context, user.id):
        ...
        if pending:
            ...
            status = await context.bot.send_message(
                chat_id=chat_id, text="🔍 Processing...", parse_mode="HTML"
            )
            progress = Progress(status)
            temp_dir = DOWNLOAD_DIR / f"dw_{uuid.uuid4().hex}"
            temp_dir.mkdir(parents=True, exist_ok=True)
            async with dl_semaphore:
                try:
                    ...
                    await progress.done_dl()
                    # FIX #11 — bot aur chat_id explicitly pass karo
                    await send_file_direct_ptb(
                        context.bot, chat_id, progress, file_path, info
                    )
                    await safe_edit(status, "✅ <b>Done!</b> Enjoy 🎉")
                except Exception as e:
                    await safe_edit(status, f"❌ <b>Failed!</b>\n\n<code>{e}</code>")
...
def ptb_webhook():
    ...
    if app and loop:
        ...
        try:
            # FIX #9 — timeout 30s; timeout pe bhi 200 OK return karo
